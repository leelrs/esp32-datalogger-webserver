...
int state=0; //STATE 0 is webserver,  STATE 1 is recording
void append_page_header() {
  webpage  = F("<!DOCTYPE html><html>");
  webpage += F("<head>");
  webpage += F("<title>Axle Strain Gauge Server</title>"); // NOTE: 1em = 16px
  ...
  webpage += F("ul{display:flex;padding:0;border-radius:0em;overflow:hidden;background-color:#d90707;font-size:1em;}");
  ...
  webpage += F("rec{background-color: green;align-self: center;}");
  webpage += F("norec{background-color: red;align-self: center;}");
  webpage += F("</style></head><body><h1>Axle Strain Gauge</h1>");
  webpage += F("<ul>");
  webpage += F("<li><a href='/'>Files</a></li>"); //Menu bar with commands
  webpage += F("<li><a href='/upload'>Configuration</a></li>"); 
  webpage += F("<li><a href='/start'>Start Recording</a></li>"); 
  webpage += F("<li><a href='/stop'>Stop Recording</a></li>"); 
  if (state==0){
    webpage += F("<norec>Not Recording</norec>"); 
  }
  if (state==1){
  webpage += F("<rec>Recording</rec>"); 
  }
  webpage += F("</ul>");
}
